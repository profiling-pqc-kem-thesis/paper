...
The domain parameters $a,b\in\mathbb{F}_q$ describe the curve $E$ as $y^2=x^3+ax+b$. Illustrations of these curves may be seen in figure \ref{figure:background:elliptic-curves}. Not all curves are suitable for elliptic-curve cryptography, which has led to some being standardized for mass usage\todo{cite}.
...
This section further describes post-quantum cryptography, the standardization process and post-quantum cryptosystems.
...
CRYSTALS-KYBER is a lattice-based cryptosystem that relies on the learning-with-errors problem over module lattices being hard to solve.
