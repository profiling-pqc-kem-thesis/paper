...
A lattice is the set of linear combinations of the basis vectors in a euclidean space. It is defined as follows, where $\mathcal{Z}$ refers to an integral coefficient and $x_1,x_2,...,x_n$ a basis in the euclidean space $\mathcal{R}^n$.

$$
L=\mathcal{Z}x_1+\mathcal{Z}x_2+...+\mathcal{Z}x_n=\left\{\sum_{i=1}^n a_ix_i|a_1,a_2,...,a_n\in\mathcal{Z}\right\}
$$

A lattice provides several applications in number theory and has been applied to cryptography, where the problems imposed by lattices is taken advantage of. The most significant problem is the Shortest Vector Problem. The problem revolves around approximating the minimal euclidean length of a non-zero lattice vector. The problem has been shown to be hard to solve efficiently and is thought to be secure from future classical and post-quantum algorithms alike.

% https://books.google.se/books?hl=sv&lr=&id=i5AkDxkrjPcC&oi=fnd&pg=PP1&dq=lattice+introduction&ots=PAW__I7tB3&sig=teKBp5L19aItLe5Y857epoxvb9s&redir_esc=y#v=onepage&q=lattice%20introduction&f=false
pp2, pp 5

% https://ieeexplore-ieee-org.miman.bib.bth.se/stamp/stamp.jsp?tp=&arnumber=9224855
...
\todo[inline]{Should we have a section here on what page faults, cycles, wall-clock time, cache misses are?}
...
\section{Consumer and Cloud Hardware}
