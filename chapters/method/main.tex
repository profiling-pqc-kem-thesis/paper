...
To identify relevant research papers, we will search for peer-reviewed papers by using databases such as Scopus and Web of Science. By searching for relevant keywords such as \textit{mainframe cryptography}, \texit{z15 cryptography}, \textit{cpacf performance}, \textit{mainframe cpu cache}, \textit{cryptocards}, \textit{z15 simd}, \texit{z15 memory security} and \textit{z15 alu} we will select papers that seem relevant based on their title and abstracts. We will then read the selected papers in their entirety to determine the quality and relevance.
...
All of the aforementioned compiler configurations will be tested in all of the environments previously listed. On all environments, the reference implementations of all the algorithms under test will be evaluated. If an environment supports \gls{avx2}, an \gls{avx2} implementation will be tested as well.
